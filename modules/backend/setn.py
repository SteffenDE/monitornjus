...
def updateurl_refresh(Name, GETNAME, Seite, Nummer, widgname, referer, form):
    if "index" in referer:
        ...
        raise Warning(
            "Function updateurl_refresh: This referer does not exist.")
...
def updateaktiv(Name, GETNAME, Seite, Nummer, widgname, hidden, referer):
    ...
    if "index" in referer:
        ...
        if val_flag == common.getwidgetinfo(widgname, Nummer, GETNAME):
            ...
            common.writewidgetinfo(
                widgname, Nummer, GETNAME, unicode(val_flag))
    else:
...
def updatetime(Seite, Nummer, referer, form):
    if "index" in referer:
        ...
        wochentag = unicode(
            form.get("wochentag-" + Seite + "-" + unicode(Nummer)))
    ...
    if "index" in referer:
            ...
            if not "teilung" in item and not "referer" in item:
                ...
                if not "uhrzeit" in item and not "wochentag" in item and not "tag" in item and not "monat" in item:
                    if not "aktiv" in name.lower():
                        updateurl_refresh(item, name, seite,
                                          nummer, "", referer, form)
                    else:
                        if "hidden." in item.lower() and not item[7:] in form:
                            hidden = 0
                            updateaktiv(item[7:], name[7:], seite,
                                        nummer, "", hidden, referer)
                        elif "hidden." in item.lower() and item[7:] in form:
                            pass
                        else:
                            hidden = None
                            updateaktiv(item, name, seite, nummer,
                                        "", hidden, referer)
                else:
            ...
            if not "referer" in item:
                ...
                if not "aktiv" in art.lower():
                    if not "url" in art.lower():
                        ...
                        updateurl_refresh(item, art, "", ID,
                                          typ, referer, form)
                else:
                    if "hidden." in item.lower() and not item[7:] in form:
                        hidden = 0
                        updateaktiv(item[7:], art[7:], "",
                                    ID, typ, hidden, referer)
                    elif "hidden." in item.lower() and item[7:] in form:
                        pass
                    else:
                        hidden = None
        ...
        if cnum is not None and cnum.isdigit():
            ...
            if num == int(common.getrows()) + 1:
                ...
                raise Warning(
                    "Neues Displayset - falsche Zahl: " + unicode(num))
            ...
            if num <= int(common.getrows()):
                ...
                raise Warning(
                    "Displayset löschen - falsche Zahl: " + unicode(num))
        ...
        if val is not None:
            if val == "Logo" or val == "Freies_Widget":
                ...
                common.newwidget(ID, val, val, 0, "placeholder",
                                 "bottom", "0px", "center", "0px", "100%", "100%")
            else:
